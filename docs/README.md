...
It helps a nontechnical merchant publish an agent-readable catalog and become transactable by an AI buyer through a narrow MCP interface, while every money action remains explainable, bounded, gated and auditable and is executed using Razorpay Test Mode.
...
One documented MCP-capable client can:
...
MCP is an interface, not a payment engine. Its tools call the same application services as the HTTP API.
...
V1 demonstrates one merchant and one MCP client. A second merchant fixture proves tenant isolation. The system is not a marketplace and does not scrape, compare or route across open-web merchants.
...
2. discover it through the configured MCP client;
