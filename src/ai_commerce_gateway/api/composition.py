...
def _import_integrated_services() -> Any:
    ...
    return SimpleNamespace(
        ...
        compose_transaction_services=compose_transaction_services,
...
def compose_in_process_buyer_services_factory(
    ...
    Mirrors the sibling lanes' reviewed composition:
    ...
    * proposals / authorizations — transaction lane application services over
      ...
      work);
    * transactions — transaction lane ``compose_transaction_services`` with
      its own execution/reconciliation/query units of work;
    * approvals — ``SessionBuyerApprovalVerifier`` binding approval to the
    ...
    provider = integrated.RazorpayAdapter(
        key_id=config.razorpay_key_id,
        key_secret=config.razorpay_key_secret.get_secret_value(),
        api_base_url=config.razorpay_api_base_url,
        timeout_seconds=config.razorpay_timeout_seconds,
        checkout_name=config.razorpay_checkout_name,
    )
    ...
    def factory() -> Iterator[BuyerServiceBundle]:
        with session_scope(session_factory) as request_session:
            ...
            transaction_services = integrated.compose_transaction_services(
...
def compose_default_buyer_services_factory(
    ...
    if config.buyer_services_mode == "in_process":
        return compose_in_process_buyer_services_factory(config)
    raise RuntimeError(
